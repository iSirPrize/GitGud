...
app.post("/api/notifications/schedule-notification", async (req, res) => {
  try {
    const { userId, notificationId, chatId, type, senderName, messageSnippet } =
      req.body;
    res
      .status(202)
      .json({ success: true, message: "Countdown initiated on server." });
    ...
    if (activeServerJobs[jobKey]) {
      ...
      console.log(
        `[Server Scheduler] Overwriting existing active timer for key: ${jobKey}`,
      );
    }
    ...
    console.log(
      `[Server Scheduler] Email scheduled for user ${userId} in 5 minutes (Key: ${jobKey})`,
    );

    const job = schedule.scheduleJob(runTime, async () => {
      try {
        console.log(
          `[Server Scheduler] 5 minutes up! Verifying Firestore status for Key: ${jobKey}...`,
        );
        ...
        if (!shouldSendEmail) {
          console.log(
            `[Server Scheduler] Target user read their alerts. Aborting email delivery!`,
          );
        ...
        if (!targetEmail) {
          console.log(
            `[Server Scheduler] No valid email found for user ${userId}. Aborting.`,
          );
          delete activeServerJobs[jobKey];
          return;
        }
          ...
          "https://api.emailjs.com/api/v1.0/email/send",
          {
            ...
            body: JSON.stringify({
              ...
              template_params: {
                ...
                action_url:
                  type === "friend_request"
                    ? `${process.env.FRONTEND_URL}/profile/${userId}`
                    : type === "comment_reply"
                      ? `${process.env.FRONTEND_URL}/posts/${quizId || chatId}`
                      : `${process.env.FRONTEND_URL}/messages/${chatId}`,
              },
            }),
          },
        );
        ...
        if (emailjsResponse.ok) {
          console.log(
            `[Server Scheduler] Backend email successfully sent to ${targetEmail}`,
          );
        } else {
          const errText = await emailjsResponse.text();
          console.error(
            `[Server Scheduler] EmailJS server returned an error:`,
            errText,
          );
        }
      } catch (jobErr) {
        console.error(
          "[Server Scheduler] Error executing background task:",
          jobErr,
        );
      }
...
app.post("/api/notifications/cancel-notification", (req, res) => {
  ...
  if (activeServerJobs[jobKey]) {
    ...
    delete activeServerJobs[jobKey];
    console.log(
      `[Server Scheduler] Cancelled active email clock for key: ${jobKey}`,
    );
    return res.json({
      cancelled: true,
      message: "Countdown aborted successfully.",
    });
  }

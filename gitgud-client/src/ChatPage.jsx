      ...
      try {
        ...
        if (!snap.empty) {
          ...
          snap.docs.forEach((d) => {
            ...
            const jobKey = d.id;
            fetch(
              "http://localhost:3001/api/notifications/cancel-notification",
              {
                method: "POST",
              ...
              console.error("Express cancellation notice dropped:", err),
            );
      ...
      for (const participantId of participants) {
        if (participantId !== user.uid) {
          ...
          if (currentUnread === 0) {
            let newNotifId = "";
            try {
              const newNotifRef = await addDoc(
                collection(db, "users", participantId, "notifications"),
                {
                  type: "unread_message",
            ...
            if (newNotifId) {
              fetch(
                "http://localhost:3001/api/notifications/schedule-notification",
                {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                ...
                console.error("Failed to alert Express scheduler:", err),
              );
            }
          }
        }
